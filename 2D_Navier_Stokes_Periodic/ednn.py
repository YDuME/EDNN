...
class EvolutionalDNN:
    ...
    def output(self, X):
        with tf.GradientTape(persistent=True) as tape1:
            tape1.watch(X)
            sinX1 = tf.reshape(tf.sin(X[:,0]),[-1,1])
            cosX1 = tf.reshape(tf.cos(X[:,0]),[-1,1])
            sinX2 = tf.reshape(tf.sin(X[:,1]),[-1,1])
            cosX2 = tf.reshape(tf.cos(X[:,1]),[-1,1])
            XT = tf.concat([sinX1,cosX1,sinX2,cosX2],axis = 1)
            Phi = self.model(XT)
    ...
    # Calculate the vorticity using automatic differentiation
    @tf.function
    def output_vorticity(self, X):
        with tf.GradientTape(persistent=True) as tape2:
            ...
            U = tf.reshape(U,[-1])
            V = tf.reshape(V,[-1])
        dU = tape2.gradient(U,X)
        dV = tape2.gradient(V,X)
        del tape2
        omega = dV[:,0] - dU[:,1]
        return omega
