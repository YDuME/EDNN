...
class EvolutionalDNN:
    ...
    def output(self, X,X_boundary):
        ...
        with tf.GradientTape(persistent=True) as tape1:
            tape1.watch(X)
            sinX1 = tf.reshape(tf.sin(k*X[:,0]),[-1,1])
            cosX1 = tf.reshape(tf.cos(k*X[:,0]),[-1,1])
            X2 = tf.reshape(X[:,1],[-1,1])
            XT = tf.concat([sinX1,cosX1,X2],axis = 1)
            Phi_hat = tf.reshape(self.model(XT)[0], [-1])
            XNaux = tf.concat([tf.reshape(tf.sin(k*X[:,0]),          [len(X),1,1]),
                               tf.reshape(tf.cos(k*X[:,0]),          [len(X),1,1]),
                               tf.reshape(         X_boundary[:,2,1],[len(X),1,1])],axis = 2)
            XSaux = tf.concat([tf.reshape(tf.sin(k*X[:,0]),          [len(X),1,1]),
                               tf.reshape(tf.cos(k*X[:,0]),          [len(X),1,1]),
                               tf.reshape(         X_boundary[:,3,1],[len(X),1,1])],axis = 2)
            with tf.GradientTape(persistent=True) as tape2, tf.GradientTape(persistent=True) as tape3:
                tape2.watch(XSaux)
                tape3.watch(XNaux)
                X_boundary_aux = tf.concat([XSaux,XNaux], axis = 1)
                output_boundary = self.model(tf.reshape(X_boundary_aux,[-1,3]))
                Phi_boundary = tf.reshape(output_boundary[0],[-1,2])
                Phi_boundary_s = Phi_boundary[:,0]
                Phi_boundary_n = Phi_boundary[:,1]
            dPhi_s = tape2.gradient(Phi_boundary_s, XSaux)
            dPhidYs = dPhi_s[:,0,2]
            dPhi_n = tape3.gradient(Phi_boundary_n, XNaux)
            dPhidYn = dPhi_n[:,0,2]
            del tape2, tape3
            fs = 2 * (X[:,1]/h)**3 - 3 * (X[:,1]/h)**2 + 1
            gs = X[:,1]**3 / h**2 - 2 / h * X[:,1]**2 + X[:,1]
            Bs = Phi_boundary_s * fs + dPhidYs * gs
            fn = 2 * ((h - X[:,1])/h)**3 - 3 * ((h - X[:,1])/h)**2 + 1
            gn = X[:,1]**3 / h**2 - X[:,1]**2 / h
            Bn = Phi_boundary_n * fn + dPhidYn * gn
            Bq = tf.cos(tf.constant(np.pi/2)*X[:,1])
            Phi = Phi_hat - Bs - Bn + Bq
