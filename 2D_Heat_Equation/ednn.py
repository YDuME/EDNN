...
class EvolutionalDNN:
    ...
    def train(self,
        ...
        for ep in range(ep0, ep0+epochs):
            for ba in range(batches):
                ...
                loss_data = self.training_step(X_batch,  X_boundary_batch, Y_batch,
                                                   alpha,
                                                   ba_counter)
    ...
    @tf.function
    def eval_NN_grad(self, X, X_boundary):
        '''
    ...
    #@tf.function
    def eval_rhs(self,Input,Input_boundary,nbatch,w=None):
        '''
        ...
        dudt = self.rhs(self.output, Input, Input_boundary, self.eq_params)
    ...
    def Marching(self, Input, Input_boundary, params):
        W = self.get_weights_np()
        W = self.marching_method(W,self.eval_rhs,Input,Input_boundary, params)
        self.set_weights_np(W)
        return None

    # A wrapper to enforce Dirichlet B.C. on a given neural network self.model 
    @tf.function
    def output(self, X,X_boundary):

        aeast = (X_boundary[:,0] - X)[:,0]
        awest = -(X_boundary[:,1] - X)[:,0]
        anorth = (X_boundary[:,2] - X)[:,1]
        asouth = -(X_boundary[:,3] - X)[:,1]
        output = self.model(X)
        D1, D2, D3 = X_boundary.shape
        XEaux = tf.concat([tf.reshape(X_boundary[:,0,0],[len(X),1,1]),tf.reshape(X[:,1],[len(X),1,1])],axis = 2)
        XWaux = tf.concat([tf.reshape(X_boundary[:,1,0],[len(X),1,1]),tf.reshape(X[:,1],[len(X),1,1])],axis = 2)
        XNaux = tf.concat([tf.reshape(X[:,0],[len(X),1,1]),tf.reshape(X_boundary[:,2,1],[len(X),1,1])],axis = 2)
        XSaux = tf.concat([tf.reshape(X[:,0],[len(X),1,1]),tf.reshape(X_boundary[:,3,1],[len(X),1,1])],axis = 2)
        X_boundary_aux = tf.concat([XEaux, XWaux, XNaux, XSaux], axis = 1)
        output_boundary = self.model(tf.reshape(X_boundary_aux,[-1,D3]))
        Y_hat = output[0]
        Y_boundary_pred = tf.reshape(output_boundary[0],[-1,D2])
        B =              (tf.reshape((awest * asouth * anorth) / (awest * asouth * anorth + aeast),(-1,1)) * tf.reshape(Y_boundary_pred[:,0],(-1,1))
                       +  tf.reshape((aeast * asouth * anorth) / (aeast * asouth * anorth + awest),(-1,1)) * tf.reshape(Y_boundary_pred[:,1],(-1,1))
                       +  tf.reshape((awest * aeast * asouth) / (awest * aeast * asouth + anorth),(-1,1)) * tf.reshape(Y_boundary_pred[:,2],(-1,1))
                       +  tf.reshape((awest * aeast * anorth) / (awest * aeast * anorth + asouth),(-1,1)) * tf.reshape(Y_boundary_pred[:,3],(-1,1)) )
        Y_pred = Y_hat - B
    ...
    @tf.function
    def training_step(self, X_batch, X_boundary_batch, Y_batch, 
                      alpha,  ba):
        with tf.GradientTape(persistent=True) as tape:
            Ypred = self.output(X_batch, X_boundary_batch)[0]
            aux = [tf.reduce_mean(tf.square(Ypred[:,i] - Y_batch[:,i])) for i in range(Ypred.shape[1])]
    ...
    def print_status(self, ep, lu,   alpha, verbose=False):
...
def get_mini_batch(X, Xb, Y, ba, batches, flag_idxs, random=True):
    idxs = []
    for fi in flag_idxs:
        if random:
